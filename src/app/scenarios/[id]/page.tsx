...
export default function ScenarioDetailsPage() {
  ...
  useEffect(() => {
    if (!isLoading && scenario && scenario.isPro && !isProUser) {
      router.push(`/upgrade?from=/scenarios/${id}`);
    }
  }, [isLoading, scenario, isProUser, router, id]);
      ...
      <main className="pt-24 pb-12 px-6 max-w-6xl mx-auto">
        ...
        <div className="mb-12 border-b border-slate-200 pb-8">
          <div className="flex items-center space-x-2 text-indigo-600 mb-3">
            ...
            <span className="text-sm font-bold uppercase tracking-widest">{scenario.badge}</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              {(['SOFT', 'NEUTRAL', 'FIRM'] as Tone[]).map((tone) => {
                 const data = scenario.tones[tone];
                 ...
                 return (
                  <div key={tone} className={`bg-white rounded-3xl p-6 border ${borderColor} shadow-sm hover:shadow-md transition-all flex flex-col justify-between`}>
                    <div className="flex justify-between items-start mb-4">
                      <span className={`text-xs font-black px-3 py-1 rounded-full ${badgeColor}`}>{tone}</span>
                      <span className="text-xs text-slate-400">{data.hint}</span>
                    </div>
                    <p className="text-lg leading-relaxed font-medium text-slate-800 mb-6">
                      &quot;{data.phrase}&quot;
                    </p>
                    <button 
                      onClick={() => handleCopy(tone, data.phrase)}
                      className={`w-full text-white py-3 rounded-xl flex items-center justify-center space-x-2 font-bold text-sm active:scale-95 transition-all ${copiedTone === tone ? 'bg-slate-900 border-slate-900' : btnColor}`}
                    >
                      <Icon icon={copiedTone === tone ? "heroicons:check-circle-20-solid" : "heroicons:document-duplicate-20-solid"} className={copiedTone === tone ? "animate-check" : ""} />
                      <span>{copiedTone === tone ? 'Copied!' : 'Copy Phrase'}</span>
                    </button>
                  </div>
                 );
              })}
